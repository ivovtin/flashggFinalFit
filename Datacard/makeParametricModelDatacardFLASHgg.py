...
if options.doSTXS:
  ...
  for proc in allProcs:
		fileDetails[proc] = 	[sigFile.replace('$PROC',proc),sigWS,'hggpdfsmrel_%dTeV_%s_$CHANNEL'%(sqrts,proc)]
else:
...
def printFileOptions():
  ...
  for typ, info in fileDetails.items():
    for c in options.cats:
      ...
      if 'DoubleHTag_10' in c or 'DoubleHTag_11' in c : outFile.write('shapes %-10s %-15s %-30s %-30s\n'%(typ,'%s_%dTeV'%(c,sqrts),file.replace('70GeV','90GeV').replace('.root','.root'),wsname+':'+pdfname))  #this is needed because we have different range for these two categories
...
def getFlashggLine(proc,cat,syst):
 ...
 # dataNOMINAL =  inWS.data("%s_%d_13TeV_%s"%(flashggProc[proc],options.mass,cat)) #Nominal RooDataSet,. May contain required weights if UP/DOWN/SYMMETRIC roodatahists do not exist (ie systematic stored as event weigths)
  ...
  print "%s_13TeV_%d_%s_%sDown01sigma"%(flashggProc[proc],options.mass,cat,syst)
  print "%s_13TeV_%d_%s_%sUp01sigma"%(flashggProc[proc],options.mass,cat,syst)
  print "%s_13TeV_%d_%s"%(flashggProc[proc],options.mass,cat)
  ...
  if (asymmetric and eventweight) : 
    #print "dataNOMINAL.numEntries()="%dataNOMINAL.numEntries() 
    data_up = dataNOMINAL.emptyClone();
    data_down = dataNOMINAL.emptyClone();
    data_nominal = dataNOMINAL.emptyClone();
    mass = inWS.var("CMS_hgg_mass")
